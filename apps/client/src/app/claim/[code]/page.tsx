...
export default function ClaimPage() {
  ...
  const sessionPseudo = (session?.user as { pseudo?: string | null } | undefined)?.pseudo ?? null;
  ...
  const submitClaim = async (body: Record<string, unknown> | null) => {
    if (claiming) return;
    setClaiming(true);
    setClaimError(null);
    ...
    try {
      ...
      const data = (await res.json().catch(() => null)) as
        | { ok?: boolean; pseudo?: string; error?: string; needsPseudo?: boolean }
        | null;
      ...
      if (res.status === 409 && data?.needsPseudo) {
        router.push(`/onboarding?redirect=${redirectParam}`);
        return;
      }

      setClaimError(CLAIM_ERRORS[res.status] ?? data?.error ?? "Erreur serveur, réessaie.");
    } catch {
  ...
  const showLoading = state.kind === "loading" || (sessionPending && !claimedPseudo);
        ...
        <div className="px-6 py-6">
          {claimedPseudo ? (
            ...
            <div className="flex flex-col gap-5">
              <ScorePreview preview={state.preview} />
              ...
              {isAuthed && sessionPseudo ? (
                <AccountClaim
                  pseudo={sessionPseudo}
                  claiming={claiming}
                  onClaim={() => submitClaim(null)}
                />
              ) : isAuthed ? (
                <OnboardingCta href={`/onboarding?redirect=${redirectParam}`} />
              ) : guestMode ? (
                <GuestForm
                  pseudo={pseudo}
                  setPseudo={(v) => {
...
function ScorePreview({ preview }: { preview: ClaimPreview }) {
  return (
    <>
...
function AccountClaim({
  ...
  return (
    <button
      ...
      disabled={claiming}
      className={[
        "flex h-12 w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold text-white",
    ...
    >
      ...
      {claiming ? "Attribution…" : `Réclamer en tant que ${pseudo}`}
    </button>
  );
}

function OnboardingCta({ href }: { href: string }) {
  return (
    <div className="flex flex-col gap-3 text-center">
      <p className="text-sm text-white/60">
        Choisis d&apos;abord un pseudo pour réclamer ce score sur ton compte.
      </p>
      <Link
        href={href}
        className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 text-sm font-semibold text-white transition-colors duration-200 hover:bg-orange-400"
      >
...
function AuthChoices({
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <Link
        href={loginHref}
        className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 text-sm font-semibold text-white transition-colors duration-200 hover:bg-white/10"
      >
      ...
      <button
        type="button"
        onClick={onGuest}
        className="mt-1 text-xs text-white/40 underline-offset-4 transition-colors hover:text-white/70 hover:underline"
      >
        Continuer en invité
      </button>
    </div>
